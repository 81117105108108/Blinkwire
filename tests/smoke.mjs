...
function send(method, params) {
  ...
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => { if (waiting.has(id)) { waiting.delete(id); reject(new Error(`timeout: ${method}`)); } }, 60000);
  });
}
...
try {
  ...
  for (const required of [
    'browser_snapshot', 'browser_click', 'browser_type', 'browser_navigate', 'browser_navigate_back',
    'browser_hover', 'browser_press_key', 'browser_select_option', 'browser_drag', 'browser_drop',
    'browser_file_upload', 'browser_fill_form', 'browser_find', 'browser_take_screenshot', 'browser_pdf_save',
    'browser_console_messages', 'browser_network_requests', 'browser_evaluate', 'browser_handle_dialog',
    'browser_resize', 'browser_tabs', 'browser_close', 'browser_wait_for', 'browser_mouse_click_xy', 'browser_scroll',
  ]) {
    check(`has ${required}`, names.includes(required));
  }
  check('snapshot schema is compact', JSON.stringify(list.tools.find((t) => t.name === 'browser_snapshot')).length < 1200);
  ...
  check('snapshot renders tree', /- heading "Hello Blinkwire"/.test(snap.text), snap.text.split('\n').slice(1, 4).join(' | '));
  ...
  const buttonRef = snap.text.split('\n').find((l) => /button "Go"/.test(l))?.match(/\[ref=(e\d+)\]/)?.[1];
  const inputRef = snap.text.split('\n').find((l) => /textbox/.test(l))?.match(/\[ref=(e\d+)\]/)?.[1];
  ...
  check('screenshot returns image', !!img && img.data.length > 100, img ? `${img.data.length} b64 chars` : 'none');
  ...
  check('stale ref gives a helpful error', badRef.isError && /snapshot/.test(badRef.text), badRef.text.split('\n')[0]);
} catch (e) {
