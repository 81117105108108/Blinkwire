...
for (let i = 0; i < 60 && !up; i++) {
  try { up = (await fetch(`http://127.0.0.1:${PORT}/json/version`)).ok; } catch { await sleep(200); }
}
if (!up) { console.log('FAIL  chrome never exposed a debugging port'); chrome.kill(); process.exit(1); }

const child = spawn(process.execPath, ['dist/index.js', `--port=${PORT}`], { cwd: root, stdio: ['pipe', 'pipe', 'pipe'] });
let stderr = '';
...
const send = (method, params) => new Promise((resolve, reject) => {
  ...
  setTimeout(() => waiting.has(id) && (waiting.delete(id), reject(new Error(`timeout: ${method}`))), 40000);
});
const callText = async (name, args) =>
  ((await send('tools/call', { name, arguments: args })).content ?? []).map((c) => c.text ?? '').join('\n');
...
try {
  ...
  check('reads the user\'s existing tab', /heading "Attached"/.test(snap), snap.split('\n').slice(1, 3).join(' | '));
  ...
  check('lists tabs of the attached browser', /\[0\]/.test(tabs), tabs.split('\n')[0]?.slice(0, 60));

  const created = await callText('browser_tabs', { action: 'new', url: 'data:text/html,<h1>Second</h1>' });
  check('opens a new tab', /Opened a new tab/.test(created), created.split('\n')[0]);

  const after = await callText('browser_status', {});
  check('current tab switched to the new one', /Second/.test(after) || /url:/.test(after), after.split('\n')[2]?.slice(0, 60));
} catch (e) {
