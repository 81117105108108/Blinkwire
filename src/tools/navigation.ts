...
import { assertText } from '../core/validation.js';
...
export const tools: ToolDef[] = [
  {
    ...
    async handler(args, ctx) {
      const raw = assertText(args.url, 'url');
      ...
      const loaded = Promise.race([
        s.cdp
          .onceFor('Page.loadEventFired', () => true, ctx.cfg.timeoutNavigation)
          .catch(() => 'timeout'),
        s.cdp
          .onceFor('Page.navigatedWithinDocument', () => true, ctx.cfg.timeoutNavigation)
          .catch(() => 'timeout'),
  ...
  },
  {
    ...
    params: {
      ...
      timeout: {
        type: 'number',
        description: 'Timeout in seconds when waiting for text (default 10)',
      },
    },
    async handler(args, ctx) {
      ...
      if (want !== undefined) assertText(want, 'text');
      if (gone !== undefined) assertText(gone, 'textGone');
      ...
      return text(
        r.ok ? `${label} — matched in ${r.ms}ms` : `${label} — timed out after ${r.ms}ms`,
        { ok: r.ok, ms: r.ms },
      );
    },
  },
...
async function step(ctx: Parameters<ToolDef['handler']>[1], delta: number) {
  const s = ctx.session;
  await s.ensure('Page');
  const { currentIndex, entries } = await s.cdp.send<{
    currentIndex: number;
  ...
  if (next < 0 || next >= entries.length) {
    return text(
      delta < 0 ? 'Already at the beginning of the history.' : 'Already at the end of the history.',
    );
  }
  const loaded = Promise.race([
    s.cdp
